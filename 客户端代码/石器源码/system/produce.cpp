...
no = -1;
	...
	case PRODUCE_LEFT_ACCELE:
		ret = ProduceAccele( 2 );
		break;
	case PRODUCE_RIGHT_ACCELE:	// ???
		ret = ProduceAccele( 3 );
		break;
	case PRODUCE_LEFT_RIGHT_ACCELE:	// ????
		ret = ProduceAccele( 4 );
		break;
	case PRODUCE_UP_DOWM_ACCELE:	// ???
		ret = ProduceAccele( 5 );
		break;
	case PRODUCE_UP_BRAKE:		// ??
		//ret = ProduceBrake( 0 );
		break;
	case PRODUCE_DOWN_BRAKE:	// ???
		ret = ProduceBrake( 1 );
		break;
		
	case PRODUCE_LEFT_BRAKE:	// ???
		ret = ProduceBrake( 2 );
		break;
		
	case PRODUCE_RIGHT_BRAKE:	// ???
		ret = ProduceBrake( 3 );
		break;
		
	case PRODUCE_LEFT_RIGHT_BRAKE:	// ???
		ret = ProduceBrake( 4 );
		break;
		
	case PRODUCE_UP_DOWM_BRAKE:	// // ???
		ret = ProduceBrake( 5 );
		break;
	case PRODUCE_4WAY_OUT:		// ???䴘??
		ret = Produce4Way( 0 );
		break;
	case PRODUCE_4WAY_IN:		// ???䴘?
		ret = Produce4Way( 1 );
		break;
